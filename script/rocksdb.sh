...
# rocksdb.sh
#
# E1 e1_rocksdb_run.sh의 Final GC/iCAT 1:1 대응판.
# YCSB RocksDB workload A/B/D 단일 실험.
#
# Usage:
#   /home/meen/iCAT/script/rocksdb.sh <module-tag> <a|b|d>
#
# Optional:
#   YCSB_THREADS=1
...
#   ROCKSDB_WORKLOAD_DIR=/home/meen/iCAT/workloads/rocksdb
#
# 주의:
...
readonly WORKLOAD_DIR="${ROCKSDB_WORKLOAD_DIR:-/home/meen/iCAT/workloads/rocksdb}"
readonly RESULT_ROOT="/home/meen/iCAT/result/rocksdb"
...
readonly DB_DIR="${MOUNT_POINT}/ycsb-rocksdb"
...
readonly DMESG_START_MARKER="iCAT_ROCKSDB_START module=${MODULE_TAG} workload=${WORKLOAD_KEY} run=${RUN_ID}"
readonly DMESG_END_MARKER="iCAT_ROCKSDB_END module=${MODULE_TAG} workload=${WORKLOAD_KEY} run=${RUN_ID}"

YCSB_HOME=""
ROCKSDB_BINDING_DIR=""
DC_FLAG="/tmp/icat_rocksdb_dc_$$.flag"
DC_PID=""
virt_started=0
run_marked=0

die() { printf 'rocksdb.sh: %s\n' "$*" >&2; exit 2; }
...
resolve_ycsb() {
    ...
    ROCKSDB_BINDING_DIR="${YCSB_HOME}/rocksdb-binding"
}

run_ycsb_load() {
    pushd "${YCSB_HOME}" >/dev/null
    ./bin/ycsb load rocksdb -s -P "${WL}" -threads "${YCSB_THREADS}" \
...
run_ycsb_run() {
    pushd "${YCSB_HOME}" >/dev/null
    ./bin/ycsb run rocksdb -s -P "${WL}" -threads "${YCSB_THREADS}" \
        -p rocksdb.dir="${DB_DIR}" \
...
validate_inputs() {
    ...
    [[ -d "${ROCKSDB_BINDING_DIR}" ]] || die "RocksDB binding not found: ${ROCKSDB_BINDING_DIR}"
}
...
main() {
    ...
    mkdir -p "${DB_DIR}"
    printf '[ROCKSDB] DB directory: %s\n' "${DB_DIR}"

    [[ "${DC_LOOP}" == "both" ]] && start_drop_caches

    write_dmesg_marker "iCAT_ROCKSDB_LOAD_START run=${RUN_ID}"
    printf '[LOAD] %s\n' "${WL}"
    run_ycsb_load
    flush_device "load-end"
    write_dmesg_marker "iCAT_ROCKSDB_LOAD_END run=${RUN_ID}"
    ...
    write_dmesg_marker "iCAT_ROCKSDB_RUN_START run=${RUN_ID}"
    printf '[RUN] %s\n' "${WL}"
    run_ycsb_run
    stop_drop_caches
    flush_device "run-end"
    write_dmesg_marker "iCAT_ROCKSDB_RUN_END run=${RUN_ID}"
